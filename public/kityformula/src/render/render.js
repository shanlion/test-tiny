    ...
    var kity = require("kity"),
        ...
        RenderComponenet = kity.createClass("RenderComponent", {
            ...
            constructor: function (kfEditor, options) {
                ...
                window.renderKity = this;
            },

            initCanvas: function () {
                var canvasContainer = this.kfEditor.requestService(
                        "ui.get.canvas.container"
                    ),
                    Formula = this.kfEditor.getFormulaClass();

                this.assembly = new Assembly(
                    new Formula(canvasContainer, this.options)
                );
            ...
            initCommands: function () {
                this.kfEditor.registerCommand("render", this, function (str) {
                    this.render(str);
                    console.log("renderui.update.canvas.viewui.update.canvas.view");
                    this.kfEditor.requestService("ui.update.canvas.view");
                });
                this.kfEditor.registerCommand("render.set.front.color", this, function (params) {
                    this.renderFrontColor(params);
                });
                this.kfEditor.registerCommand("render.set.front.family", this, function (params) {
                    this.renderFrontFamily(params);
                });
                this.kfEditor.registerCommand("render.set.front.bold", this, function (params) {
                    this.renderFrontBold(params);
                });
                this.kfEditor.registerCommand("render.set.front.italic", this, function (params) {
                    this.renderFrontItalic(params);
                });
            ...
            relocationToCenter: function () {
                ...
                this.formula.container.setTranslate(
                    -formulaSpace.width / 2,
                    -formulaSpace.height / 2
                );
                this.setCanvasToCenter();
            },
            ...
            selectGroup: function (groupId) {
                var groupObject = this.kfEditor.requestService(
            ...
            selectCurrentCursor: function () {
                var cursorInfo = this.kfEditor.requestService(
                        "syntax.get.record.cursor"
                    ),
                    group = this.kfEditor.requestService(
                        "syntax.get.group.object",
                        cursorInfo.groupId
                    ),
                    box = null,
            ...
            tintCurrentGroup: function () {
                var groupId = this.kfEditor.requestService(
                        "syntax.get.record.cursor"
                    ).groupId,
                    ...
                    isPlaceholder = this.kfEditor.requestService(
            ...
            render: function (latexStr) {
                var parsedTree = this.kfEditor.requestService(
                    ...
                    console.log('renderrender渲染更新');
                    if(window.svgColor) {
                        this.renderFrontColor(window.svgColor);
                    }
                    if(window.svgFontFamily) {
                        this.renderFrontFamily(window.svgFontFamily);
                    }

                    if(window.svgFontBold) {
                        this.renderFrontBold(window.svgFontBold);
                    }
                    if(window.svgFontItalic) {
                        this.renderFrontItalic(window.svgFontItalic);
                    }
                // 更新语法模块所维护的树
                this.kfEditor.requestService("syntax.update.objtree", objTree);
            },
            renderFrontColor: function(color) {
            
                window.svgColor = color
            ...
            renderFrontFamily: function(family) {
                window.svgFontFamily = family
            
                var textElements = document.querySelectorAll(
                    ".kf-editor-canvas-container svg text"
                );
                textElements.forEach(function (svg) {
                    if(svg.getAttribute('data-not-default-font')) {
                ...
                // window.__p[3].value.defaultFont = family;
            },

            renderFrontBold: function(bold) {
                console.log('renderFrontBoldrenderFrontBoldrenderFrontBold',bold);
                window.svgFontBold = bold
            ...
            renderFrontItalic: function(italic) {
            
                window.svgFontItalic = italic
            ...
            setFontStyle: function (name) {
                var viewPort = this.formula.getViewPort();
                console.log(viewPort, this.formula);
                this.formula.node.setAttribute("class", name);
            },
            ...
            clearCanvasTransform: function () {
                ...
                canvasInfo.contentOffset =
                    this.formula.container.getTranslate();
